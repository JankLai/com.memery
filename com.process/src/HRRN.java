...
public class HRRN {
	...
	public HRRN(int processCount, Queue<Process> processQueue) {
	...
	public void setHRRN() {
		...
		while (mExecutedQueue.size()<mProcessCount||!mReadyProcesses.isEmpty() || !mUnreachQueue.isEmpty()) {// 就绪进程或主进程池非空
			...
			if (!mReadyProcesses.isEmpty()) {
				...
				//对就绪队列按【serveTime】排序
				//waittingTime=currTime-arriveTime
				//R=(waittingTime+serveTime)/serveTime
				else {
					int flag=0;
					int waittingTimeFlag=currTime-mReadyProcesses.get(0).getArriveTime();
					double rateFlag=waittingTimeFlag/mReadyProcesses.get(0).getServeTime();
					for(int i=1;i<mReadyProcesses.size();i++) {
						int waittingTime=currTime-mReadyProcesses.get(i).getArriveTime();
						double rate=waittingTime/mReadyProcesses.get(i).getServeTime();
						if(rate>rateFlag) {
							flag=i;
							rateFlag=rate;
						}
